...
export async function getEvents(featured?: boolean) {
  let query = supabase.from('events').select('*').eq('is_published', true).order('event_date', { ascending: false })
...
export async function getGallery(category?: string) {
  let query = supabase.from('gallery').select('*').eq('is_published', true).order('sort_order', { ascending: true })
